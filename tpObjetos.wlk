object rolando{
	...
	const property artefactos = [espadaDelDestino, collarDivino, armadura, mascaraOscura, espejoFantastico]
	...
	method mejorArtefacto() = self.artefactos().filter({artefacto => artefacto != espejoFantastico}).max({artefacto => artefacto.unidadesDeLucha(self)})	
}
